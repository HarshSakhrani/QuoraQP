...
class BERTOnly(nn.Module):
  def __init__(self,preTrainedBert,embeddingDimension=768,numClasses=1):
    ...
    self.fc1=nn.Linear(self.embDim*2,1)

  def meanPooling(self,input):
  ...
  def forward(self,input1,input2):
    output1=self.meanPooling(input1)
    output2=self.meanPooling(input2)
...
kimcnn,avgTrainLoss,avgValidLoss,avgTrainAcc,avgValidAcc=train_model(bertOnly,12)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('BertOnly_Complete_Siamese_Loss.png', bbox_inches='tight')
plt.close()
#plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('BertOnly_Complete_Siamese_Acc.png', bbox_inches='tight')
plt.close()
...
f=open("BertOnly_Complete_Siamese_Results.txt","a")
f.write(str(cm))
f.close()
